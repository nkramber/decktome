...
## PR-27, what the branch holds (2026-09-06)
...
4. Branch `pr-27` holds PR-27, the feedback harvest (D-561), and its free gate passes. The owner triggers the workflow `smoke`, reads the checks, and merges (D-313). Then PR-28, the feedback loop, after the first real feedback (D-557), by `docs/reference/feedback-2026-09-06.md`, with OQ-70 to OQ-72 answered (D-559). PR-22 is merged as #72 (D-551). The deploy follows `docs/setup-gcp.md` on the owner's account, and the deploy half of `docs/reference/pr22-gate-2026-09-06.md` fills in after it. PR-23 is merged as #73 (D-554). PR-25 waits for the deploy (D-555). Then PR-25, the installable web app (D-547), and PR-26, the return channels, once the owner answers OQ-67. OQ-45 held the store of the allowlist, and D-420 answered it: one Firestore document, `config/allowlist`, written by `make allow EMAIL=...`. Then PR-23.
...
- Toolchain: Go 1.27.0, Node 22.23.2, pnpm 9.2.0, firebase-tools 14.14.0, Java 17, Playwright 1.62.1 with its Chromium headless shell (`playwright install chromium`).
