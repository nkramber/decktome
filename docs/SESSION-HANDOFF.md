...
**The review.** Gitar found three faults on `2914524` and `46d47ca`. The gate skipped a merge commit, a test called the real `gh`, and the label read the last commit alone. Each fix has a test that fails on the old code, and Gitar approved `cc386b4`. The Codex record read `cc386b4` as `Changes required`: the template title failed REF 1. `docs/reviews/pr-212-response.md` holds the answer. Codex then approved `568ed53`. The roadmap mark was still missing, and a mark after the approval moves the effective head. So the owner chose the order of what-you-carry and a `pr-contract` rule (D-822). Gitar approved `f500798`. The Codex record then read `f500798` as `Changes required`: the mark check matched `#2120` for #212. It had full merit, and the check now needs a digit boundary. Next: Gitar, then a repeat Codex review.
