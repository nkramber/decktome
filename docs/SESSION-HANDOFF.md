...
- Branch `pr22-decisions`, from `main` at 3765983, holds records alone (2026-09-05). They are the merge of #69 (D-543), the answer to OQ-65 (D-544), and the close of the reword guard question (D-545). PR-22 builds the direct Cloud Run origin, and `docs/setup-gcp.md` section 13 follows. The guard stays at 0.60 after a measurement of all 60 M-5 items with the guard's own overlap function. Nothing is committed. PR-22 starts after this branch merges, on a new branch from `main` (D-494).
