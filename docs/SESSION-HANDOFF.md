...
- Branch `tier0-snapshot`, from `main` at cd317c2, holds the third of the three items of D-531 (2026-09-05, D-521, D-542). `deck-gate -dry -trim DIR` writes the snapshot cut to the cards a dry run reaches. The fixture at `go/cmd/deck-gate/testdata/snapshot` holds 4,871 cards and 7,140 printings in 3.9 MB. `TestTrimmedSnapshotBuildsEveryShortlist` is the free dry lane of the deck gate, in the Go job of CI. `make deck-gate-dry` runs it by hand, and `make deck-gate-trim` rewrites the fixture after a prompt or a fixture changes. The tree is green: Go build, vet, the race suite, golangci-lint, and `make ste-check`. Nothing is committed, and the branch waits for the owner's commit and PR. CAUTION: the trimmed index differs from the full one where the theme search reads the whole corpus (D-411). The lane proves that every prompt builds and resolves, and not which cards a paid run picks.
...
1. The three work items of 2026-09-04, one branch each (D-531). The first, the precon check, merged as #67 (D-532, D-533). The second, the terse conversations (D-522, D-534 to D-539), merged as #68 (D-541). The third, the trimmed snapshot (D-521, D-542), sits on branch `tier0-snapshot` and waits for its commit and PR. The precon check ignores basic lands (D-523, F-35). The 47 terse conversations join the bar (D-522). One paid run of about $0.28 rebases the gate with the new threshold. A trimmed card snapshot of at most 10 MB serves Tier 0 (D-521).
