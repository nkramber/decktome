...
**The review.** Gitar reviewed `2914524` and asked for changes, with two findings. Both had merit. A clean merge commit listed no file, so the gate skipped it, and the documented `git log` rule returned it. The fix lets a merge move the effective head and adds `--effective-head`. The `gather` test of `ci_skip.py` called the real `gh`, and the fix stubs it. CI also failed `ref-check` on the local settings file that git ignores, and the skill now names it without backticks. Gitar then reviewed `46d47ca`, closed both, and found a third: the fix reused the name `files`, so the label read the last commit alone. A new test fails on `46d47ca`, and the fix renames the list. Gitar approved `cc386b4` at 03:49:01 UTC, 3 of 3 findings closed and no open thread. The review is current, because the push came at 03:47:32 UTC. The Codex record `77e6728` read `cc386b4` as `Changes required`, with one P2 finding: the template title `# PR-<number> review` failed REF 1. It had full merit. The template now reads `# Pull request <number> review`, and a test runs the filled skeleton through `ref_check`. `docs/reviews/pr-212-response.md` holds the answer. Next: Gitar on the new head, then a repeat Codex review (D-821).
