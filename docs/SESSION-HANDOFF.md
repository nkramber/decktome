...
- Branch `precon-basic-lands`, from `main` at 9843493, holds the first of the three items of D-531 (2026-09-05, D-532, F-35). The whole-precon check reads the nonbasic printings alone, in the chat hints and in the deck gate. It uses the basic land test the exclusion already had. The three hand-added rows of the Avengers fixture are gone. The dry run of prompt 25 reads the binder as the whole product: 57 cards excluded, 27 spare, as in run 16. The tree is green: Go build, vet, the race suite, golangci-lint, and `make ste-check`. The branch also carries the M-5 documents of 2026-09-04 and 2026-09-05. Commit 31d255d holds it all, and PR #67 is open against `main`. Next: the merge on the owner's read of the checks. Then the terse conversations (D-522) and the trimmed snapshot (D-521), one branch each from `main`.
