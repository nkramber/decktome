...
4. PR-22, the deploy to GCP for invited users (D-310, D-314, D-544). The code is PR #72 on branch `pr-22` (D-550), and the deploy follows `docs/setup-gcp.md` on the owner's account. Then PR-23. Then PR-25, the installable web app (D-547), and PR-26, the return channels, once the owner answers OQ-67. OQ-45 held the store of the allowlist, and D-420 answered it: one Firestore document, `config/allowlist`, written by `make allow EMAIL=...`. Then PR-23.
