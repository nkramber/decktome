...
- Branch `set-fixes`, from `main` at c46d97d, holds the Marvel set fix (D-518, F-40, F-41) as commit 07856c2, pushed. A bonus sheet never leads a set family beside a product, and the set and precon questions join their choices with "or". "Marvel" resolves to Marvel Super Heroes alone, and Marvel's Spider-Man stays out by the owner's word. The owner answered OQ-64 the same day (D-525), and the branch holds the group request too. Its parts: `set_groups` in the classify role at prompt version 18, and `ResolveGroup` in the set table. `GATE_ARGS` in the Makefile and probe 109 of the question gate complete it. Probe 109 ran on the owner's word on 2026-09-04 as question gate run 36, $0.0016 and 11 seconds. The classify model marked the group request live, and the sets expectation held: msc, msh, pspm, spe, and spm. The run exposed F-42. A partial run read FAIL on the two whole-set bars. Then `make eval-check` took it as the run of record against run 35. The fix is commit bf6ce2b (D-526), and PR #66 is open against `main`. The owner chose a rerun, and run 37 reads PASS as a partial run for $0.0011, with the sets expectation met again. The run 36 file predates the fix, so the owner chose the one-field patch of its header too, `"only":"109"` (D-527). `make eval-check` lists runs 36 and 37 as partial and reads green. The prompt version moved, so the next full question gate run re-baselines (D-66).
...
1. The merge of PR #66, branch `set-fixes`, on the owner's read of the checks. The branch holds the Marvel fix, the group request, runs 36 and 37, and the partial-run fix of F-42. The tree is green: Go build, vet, the race suite, golangci-lint, `make ste-check`, and `make eval-check`.
2. The M-5 scoring session (D-524). The owner scores the 30 unscored items by phone, one per exchange, and the session records each one in `docs/reference/pr7-m5-scoring.md`. Then `make m5-report` sets the fit threshold of D-423 from all 60 scores.
3. The three work items of 2026-09-04, one branch each. The precon check ignores basic lands (D-523, F-35). The 47 terse conversations join the bar (D-522). One paid run of about $0.28 rebases the gate with the new threshold. A trimmed card snapshot of at most 10 MB serves Tier 0 (D-521).
