...
- Branch `terse-bar`, from `main` at 4ee191c, holds the second of the three items of D-531 (2026-09-05, D-522, D-534 to D-539, F-43 to F-47). The 47 terse conversations are gate conversations with expectations. `GateSize` is 77, `CatalogOnlyBar` is 64, and `questions.MinGateSize` stays at 30 for the offline half. The conversation set is dated 2026-09-05. Five paid runs read the change, $0.85 in all. Run 38 read 13 misses: nine expectation rows, fixed, and four defects with a rule and a test each (D-535, F-43 to F-46). Runs 39, 40, and 41 each read one miss. Each one was the classifier with a value the message never named. The three: five colors on "Bracket 3." (D-536), a budget of 2 on "under budget" (D-537), and both spellings of a corrected commander in one turn (D-538). Run 42 passes every bar with no miss: 73 of 74 catalog-only against 64. It is the questions baseline now (D-539), and `make eval-check` reads green. The tree is green: Go build, vet, the race suite, golangci-lint, and `make ste-check`. Nothing is committed, and the branch waits for the owner's commit and PR. CAUTION: the bar is strict, and the classifier is noisy (D-230). A miss on a later run gets the same read: a row error, or a defect that gets a rule. No tolerance joins the bar without the owner's word.
- `docs/setup-gcp.md` is the guide from the domain to the deployed app (2026-09-05, on branch `terse-bar`). It reads the repo, the Google Cloud free-tier document, and dated third-party price pages, and section 17 names every source. Its estimate covers five users and three decks a week each. The month costs $8 to $20 in model calls, $0 in Google Cloud inside the free tiers, and $1 to $2 for the domain. It raised OQ-65: the Hosting rewrite to Cloud Run has no documented timeout, and a deck build streams for minutes. The owner chooses the API path of PR-22.
...
1. The three work items of 2026-09-04, one branch each (D-531). The first, the precon check, merged as #67 (D-532, D-533). The second, the terse conversations (D-522, D-534 to D-539), sits on branch `terse-bar`. Run 42 passes and is the baseline, and the branch waits for its commit and PR. The precon check ignores basic lands (D-523, F-35). The 47 terse conversations join the bar (D-522). One paid run of about $0.28 rebases the gate with the new threshold. A trimmed card snapshot of at most 10 MB serves Tier 0 (D-521).
2. The reword guard of D-88 at 0.60 overlap, on the owner's word, after the three items (D-531). 7 of 29 scored refusals were better than their row. 2 of them sit on the locked row, which left under D-260. On live rows the count is 5 of 27. A looser guard needs one question gate run of about $0.19.
3. The M-5 sheet is complete (D-529, D-530), and the fit threshold stays at 0.35: no candidate meets the 80 percent floor.
4. PR-22, the deploy to GCP for invited users (D-310, D-314), and PR-23. OQ-45 held the store of the allowlist, and D-420 answered it: one Firestore document, `config/allowlist`, written by `make allow EMAIL=...`. Then PR-23.
...
6. Every paid run goes through `eval sweep` or its Makefile target. Each one writes its run file beside the document. The bracket rejudge of the bracket 5 decks is still open from the sweep of 2026-09-03. Ask the owner before each one.
...
8. PR-22 and PR-23 in order after PR-15, one gate each (D-511).
