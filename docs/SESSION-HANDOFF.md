...
1. The commit of branch `set-fixes` and its PR. The branch holds the Marvel fix, the group request, runs 36 and 37, and the partial-run fix of F-42. The tree is green: Go build, vet, the race suite, golangci-lint, `make ste-check`, and `make eval-check`.
