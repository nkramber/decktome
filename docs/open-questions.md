...
| OQ-47 | Does the deck grid filter by power, and does the filter belong on the server? | Section 4.1 of the UI plan asks for it, and section 4.2 names no field. `PowerLevel` is a oneof, so a server filter needs two fields. | Before the PR-17 web slice. |
