...
The loop reads the eval report and the lessons file, lets a fixer agent
edit the code, runs a new gate and a new eval, and keeps each change the
fixer made only when its own rows got better (D-181). It commits to a
branch of its own and it pushes nothing.
...
1. Add one line to `.env`:
   ...
   and not to the plan. The second key is withheld so the fixer can not
...
2. Keep a container branch, and name it with `--base`. `pr-7c` is the one
   ...
   commit, so two nights rewrite the same rows and never add up
   (D-142).
...
1. The fixer reads the last eval report, the owner questions, and the
   lessons file. It commits each change on its own, with a `Rows` trailer
   and a `Hypothesis` trailer (D-181).
2. The loop checks the frozen paths, strips any attribution line from the
   commit messages, and builds the tree.
3. The gate and the eval run. About $0.25 and 33 to 35 minutes.
4. `tune-check` compares the two runs question by question and charges
   every moved question to the change that declared its row.
5. Every change kept: one commit `v0.N`. Every change dropped: the tree
   reverts. Some kept: the loop cherry-picks the kept commits, runs the
   gate again on the conversations their rows touch, folds that eval into
   the baseline, and commits `v0.N` if the merged run holds.
...
| `--noise` | 3 | Bad questions two runs of the same code may differ by (D-183). |
...
`go/internal/questions/lint.go`, `go/internal/questions/lint_test.go`,
`go/cmd/questions-gate/conversations.json`, `scripts/autotune.sh`,
`scripts/autotune-fix.sh`, `docs/owner-questions.md`,
...
The loop can not tell a change from the judge's noise when the change
moves fewer than three bad questions (D-183). It can not measure a user
the 104 conversations never imagined.
