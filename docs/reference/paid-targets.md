...
- The flag: `make codex-review PR=<n> -- --skip-gitar-review` reads no Gitar pass, and it refuses an open review thread or an issue on the Gitar dashboard alone (D-838). The Gitar pause ended (D-899), so a Codex review runs with no flag, after the Gitar pass (D-823).
