...
The three evals of the PR-7B gate started on 2026-08-26. The loop started seven times and kept nothing. One iteration was accepted and then judged wrong (D-171), and the rest were rejected. `autotune-readme.md` holds the commands.
...
The fixer agent is not in that table. It reads a report and edits Go, and its own tokens are billed to the owner somewhere else. A frontier model at real work over twelve iterations plausibly costs **ten to forty dollars**. That is more than the loop budget by a wide margin.
...
Every third conversation is held back. The fixer reads the failures of the other two thirds and never sees the holdout. Its checkout holds `.local/tune/<label>.fixer.json`, the summary with every holdout verdict removed (D-271). The loop then reads the holdout ratio, and not the ratio it showed the fixer.
...
Two whole-run guards sit over that. The holdout may not rise by more than the noise margin over the best accepted run of the night (D-271). The whole set may not either. The last run is not the reference, because a chain of small losses would then pass one margin at a time. A commit with no `Rows` trailer is rejected, because nothing can be charged to it.

A third guard reads the rows no change declared. When those get worse by more than the margin, a change touched what it did not name, and nobody can be charged.
...
The report alone was run first, as this section once asked, and it found what the batch sweep had found by hand. Then the loop started seven times, kept nothing, and taught three things. The checker judged below the noise floor. A rejected iteration left nothing behind. Two fixers tried one dead end twice. D-180 to D-184 are the answer to each.
