...
**PR-29 closed without merging.** The owner read these numbers and closed pull request #125 on 2026-09-10 (D-652). This document stays because its findings hold whatever happened to the code: F-94 rests on it, and the two features it names must not be built again.

The raw run files of the measurement are not in the repository. Every number below comes from them, and a rerun of the gate reproduces the control column.
