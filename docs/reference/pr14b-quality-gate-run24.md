...
Run date: 2026-09-15. Card snapshot: 2026-09-04. Model: 20260915T054345Z.
...
| modern | 24081 | 7773 | 3115 | 0 | 10888 | 0.99 of 98711 (bar 0.90) | 0.99 of 1635 (bar 0.95) | 0.86 of 99754 | 0.55 |
...
| Commander broken copies graded bad (D-674) | 4167 of 5907 |
| Built decks graded bad | 10 of 25 |
| Judge agreement | 10 of 25 |
...
| 24 | a 60-card deck from one set | Modern | bad | bad | yes |
...
| `land` | 30.532 | 5.542 | -0.283 |
| `avg_mana_value` | 2.785 | 0.859 | -0.451 |
| `color_sources` | 1.078 | 0.308 | +0.007 |
| `tapped_share` | 0.071 | 0.083 | +0.362 |
...
| `ramp` | 17.895 | 7.625 | +0.041 |
| `draw` | 7.377 | 4.096 | -0.226 |
...
| colors | 0.92 of 27525 | 863 | 587 | 749 | 178 of 178 | 0, 0, 0 | `card_rate` 0.00, `synergy` 0.00, `unseen_share` 0.00 | `source_spread` 3.70, `tapped_share` 2.76, `color_sources` 1.99 |
| synergy | 0.78 of 97485 | 7462 | 6671 | 7114 | 193 of 228 | 25, 6, 4 | `card_rate` 0.09, `synergy` 1.23, `unseen_share` 0.95 | `synergy` 1.23, `unseen_share` 0.95, `wipe` 0.68 |
...
- bad: [4167 1340 394 6 0]
- baseline: [62 71 56 0 0]
- typical: [72 276 634 48 10]
- good: [264 76 234 1441 1985]
- great: [255 89 206 1345 2105]
...
| Feature | Mean | Std | Weight |
|---|---|---|---|
| `card_rate` | 0.065 | 0.045 | +0.529 |
| `unseen_share` | 0.218 | 0.365 | -0.543 |
| `synergy` | 1.409 | 0.999 | +1.489 |
| `land` | 21.640 | 3.103 | -0.089 |
| `avg_mana_value` | 2.751 | 0.946 | -0.331 |
| `color_sources` | 0.889 | 0.363 | +0.247 |
| `tapped_share` | 0.125 | 0.141 | +0.089 |
| `mana_turn_four` | 3.843 | 0.800 | -0.028 |
| `hands_two_to_four_lands` | 0.731 | 0.061 | +0.358 |
| `curve_low` | 0.583 | 0.219 | -0.172 |
| `curve_high` | 0.181 | 0.159 | -0.304 |
| `ramp` | 5.396 | 6.781 | +0.118 |
| `draw` | 4.630 | 4.504 | +0.044 |
| `removal` | 7.817 | 4.559 | -0.200 |
| `wipe` | 1.021 | 1.761 | +0.004 |
| `interaction` | 3.082 | 3.880 | +0.044 |
| `empty_roles` | 1.020 | 0.837 | -0.036 |
| `fast_mana` | 0.581 | 1.527 | -0.019 |
| `game_changer` | 0.270 | 0.885 | -0.134 |
| `playset_share` | 0.575 | 0.235 | -0.553 |
| `singleton_share` | 0.144 | 0.161 | -0.720 |
| `source_spread` | 0.249 | 0.222 | -0.272 |

Thresholds: -3.692, -1.479, 0.815, 2.368. Loss: 1.066 over 3000 iterations. Defect detector accuracy on the holdout: 0.93, at a cut of 0.42.

The precon bar per broken axis, each precon over its own copies, with the cross pairs after it:

- lands: 1.00 of 327, cross 0.93 of 41250
- curve: 1.00 of 327, cross 0.98 of 41250
- colors: 0.97 of 75, cross 0.47 of 20632
- copies: 1.00 of 61, cross 0.87 of 23234
- synergy: 0.98 of 845, cross 0.85 of 74904

The misses per axis over the folds (M-7). Both passed: the detector passed the precon and the copy, and the ladder put the copy at or above the precon. Precon flagged: the detector flagged the precon and passed the copy. Both flagged: it flagged both and read the precon as the more broken. Own copy: each precon against its own copy on the axis, with its misses by the same kinds. Moved: the mean absolute standardized delta between a precon and its own copy, for the corpus features and the three features that move most.

| Axis | Share | Both passed | Precon flagged | Both flagged | Own copy | Own misses | Corpus moved | Most moved |
|---|---|---|---|---|---|---|---|---|
| lands | 0.93 of 41250 | 1636 | 516 | 904 | 327 of 327 | 0, 0, 0 | `card_rate` 0.01, `synergy` 0.00, `unseen_share` 0.09 | `land` 2.54, `hands_two_to_four_lands` 2.40, `color_sources` 1.33 |
| curve | 0.98 of 41250 | 198 | 88 | 551 | 327 of 327 | 0, 0, 0 | `card_rate` 0.03, `synergy` 0.00, `unseen_share` 0.20 | `curve_high` 2.67, `avg_mana_value` 2.23, `curve_low` 1.72 |
| colors | 0.47 of 20632 | 7893 | 2464 | 666 | 73 of 75 | 2, 0, 0 | `card_rate` 0.00, `synergy` 0.00, `unseen_share` 0.00 | `tapped_share` 1.25, `color_sources` 0.99, `source_spread` 0.95 |
| copies | 0.87 of 23234 | 1768 | 645 | 534 | 61 of 61 | 0, 0, 0 | `card_rate` 0.04, `synergy` 0.00, `unseen_share` 0.20 | `singleton_share` 3.49, `playset_share` 2.40, `empty_roles` 0.76 |
| synergy | 0.85 of 74904 | 5838 | 2777 | 2967 | 825 of 845 | 14, 0, 6 | `card_rate` 0.03, `synergy` 0.00, `unseen_share` 0.21 | `playset_share` 1.32, `empty_roles` 0.63, `removal` 0.61 |

The precons that lose most over the sampled pairs of the bar:

| Precon | Grade | Detector | Misses |
|---|---|---|---|
| Sultai Schemers (2014-09-26) | bad | 0.89 | 224 of 344 |
| Jeskai Monks (2014-09-26) | bad | 0.83 | 193 of 333 |
| Thallids (2005-08-01) | bad | 0.82 | 129 of 240 |
| Beasts (2005-08-01) | bad | 0.86 | 127 of 240 |
| Grave Advantage (2015-01-23) | bad | 0.77 | 114 of 240 |
| Illusionary Might (2011-08-12) | bad | 0.82 | 114 of 240 |
| Stampede (2004-06-04) | bad | 0.79 | 147 of 327 |
| Wizards (2005-08-01) | bad | 0.81 | 138 of 307 |
| Spiraling Doom (2012-02-24) | bad | 0.78 | 129 of 307 |
| Second Sun Control (2018-04-06) | bad | 0.70 | 132 of 333 |

| Axis | Own pairs | Lost | Mean rank, won | Mean rank, lost | `card_rate` move | `synergy` move | Copy graded above |
|---|---|---|---|---|---|---|---|
| lands | 327 | 0 | 32.83 | - | - | - | 0 of 0 |
| curve | 327 | 0 | 32.83 | - | - | - | 0 of 0 |
| colors | 75 | 2 | 37.56 | 28.00 | 0.00 | 0.00 | 0 of 2 |
| copies | 61 | 0 | 37.18 | - | - | - | 0 of 0 |
| synergy | 845 | 20 | 32.18 | 29.50 | 0.05 | 0.00 | 1 of 20 |

Confusion on the holdout over the folds, rows are the label and columns the grade, worst first:

- bad: [2906 60 135 11 3]
- baseline: [206 116 5 0 0]
- typical: [50 3 87 103 53]
- good: [179 1 885 1422 1513]
- great: [108 0 667 876 1499]
...
- Suite `quality`, run `pr14b-quality-gate-run24`, on 2026-09-15, commit `76e53c7`.
- Roles: none, no provider call.
- Versions: card snapshot 2026-09-04, commanders_day `2026-09-14`, quality_model `20260915T054345Z`.
- Calls: 0. Cost: unpriced. Time: 144 seconds.

## Verdict

Verdict: PASS. Time: 144 seconds, no provider call.
