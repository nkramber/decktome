...
| F-51 | **The pair bar of the quality gate reads a subset of the precons.** `MaxPairChecks` caps a bar at 20,000 pairs, and the loop walks the holdout precons in order. Commander holds 50,600 pairs, so the bar reads the first 17 of 44 precons, and Modern the first 37 of 61. The Commander bar reads 0.78 against a weighted mean of about 0.87 over the axis bars. Found 2026-09-07 in the read of run 13. Binds M-7. | 🔧 planned |
| F-52 | **The Standard precon bar rests on one holdout precon.** Standard holds 8 precons inside its three-year pool (D-478), 7 in train and 1 in the holdout, and the bar reads 50 pairs of one deck. Found 2026-09-07 in run 13. Binds M-7. | 🔧 planned |
| F-53 | **The corpus weighs the casual rungs zero, so the ladder measures the distance to a tournament list.** `tierWeight` gives a typical or a baseline list 0, and the rates and the pairs come from the great and the good lists alone. A precon's cards mostly read the prior, and its synergy-broken copy reads much the same, so the Commander synergy axis reads 0.78. The four Commander weights against sense, the 19 of 24 built decks graded bad, and the judge bar at 7 of 25 share the cause. Found 2026-09-07 in the read of run 13 and `corpus.go`. Binds PR-29, PR-30. | 🔧 planned |
| F-54 | **The colors break starves the first color of the identity, not the color the spells need, and two copies features measure one thing.** The break turns the lands that fix into basics of the first color in W-U-B-R-G order. `playset_share` and `singleton_share` both weigh negative in Modern, and the copies break moves them in opposite directions. Modern reads colors 0.51 and copies 0.89, and Standard colors 0.38 of 8. Found 2026-09-07 in run 13 and `synthetic.go`. Binds PR-31. | 🔧 planned |
...
**M-7: The axis diagnostic and the honest bars (F-51, F-52, F-53, OQ-73).** 🔧 planned, first of the weak-axes plan, `docs/reference/weak-axes-2026-09-07.md`.
...
**PR-31: The colors and the copies axes of the 60-card formats (F-54).** 🔧 planned, after M-7. It does not depend on PR-29.
...
26. **M-7** the axis diagnostic and the honest bars, then **PR-29** the casual corpus. Then **PR-30** the commander reference on OQ-74, and **PR-31** the 60-card axes (D-567). The plan is `docs/reference/weak-axes-2026-09-07.md`, and the work runs now, during the deploy wait (D-568).
