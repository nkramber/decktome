...
| F-49 | **No channel carries a user's verdict on a question, a summary, a card, or a deck.** The owner told the agent "this question was already answered", "this card does not belong in this deck", and "this deck is inaccurate against my specs" many times in the sessions of 2026-08 and 2026-09, and every one of those reached a fix by hand. An invited user has no way to say the same, and nothing reads it. Found 2026-09-06 when the owner asked for the feedback system (D-557). Binds PR-27, PR-28. PR-27 merged as #75 on 2026-09-07 (D-562), and PR-28 waits for the first real feedback. | 🔧 planned |
...
**PR-28: The feedback loop (F-49, D-557 to D-559).** 🔧 planned, after PR-27 and the first real feedback. Stage two of the design note.
`make feedback-harvest` reads the new feedback with the owner's credentials and writes a dated document and a JSONL file. Each item carries the session turns, the slots, the deck list, the session id, and the uid, never the email (D-559).

The judge role names one of ten classes for each thumbs down. Three are about a question: asked again, off target, and bad options. One is a false summary. Three are about a card: off theme, illegal, and an unwanted buy. Three are about the deck: off spec, a bad mana base, and a bad power level.

Each class writes one artifact. A question class writes a golden conversation with a "must not ask" expectation. A card or a deck class writes a deck gate prompt with an assertion, a summary judge case, or a rules test. A thumbs up becomes a keep case that a change must not flip.

The fixer agent of the tuning loop then runs on a branch of its own with the new cases. The gate that owns the cases measures the change, under a cap. Every guard of `docs/reference/autotune-design.md` holds, and one more: a class that meets an owner decision writes a row to `docs/owner-questions.md` instead of a fix. The result of a cycle is a pull request with its evidence, and the owner merges or closes it. No step runs in CI (D-286).

Gate: a harvest over the emulator with ten seeded items writes the document and the cases. A dry triage over ten fixture items, the owner's own past complaints, names the expected class of each. One live cycle on the owner's word ends with a pull request and a passing gate on its new cases. `make eval-check` shows no flip on the baselines.
> *In plain English:* the complaints become test cases, and an assistant tries to fix what they show. It works on a copy, proves each fix against the tests, and hands you a change to approve. Nothing ships without you.
...
**M-7: The axis diagnostic and the honest bars (F-51, F-52, F-53, F-55, OQ-73).** 🔧 built 2026-09-07 on branch `m7-honest-bars` (D-571), first of the weak-axes plan, `docs/reference/weak-axes-2026-09-07.md`. The build found and fixed F-55 on the way. Quality gate run 14 is its gate.
...
**PR-33: One model call that lands in band (F-77, F-78).** 🔧 built 2026-09-08. `docs/reference/on-band-plan-2026-09-08.md` holds the read, the four causes, and the plan. Every part is free.
...
19. **Phase 3B** (D-316, D-317): PR-16 to PR-23 in the order of the phase list. PR-14A and PR-14B sit between PR-19 and PR-20 (D-452, D-460). PR-14A ✅ merged 2026-09-02 (#57). PR-14B ✅ merged 2026-09-03 (#58). PR-19 ✅ merged 2026-09-02 (#55, #56). PR-20 ✅ merged 2026-09-03 (#61). PR-21 ✅ merged 2026-09-03 (#62). PR-22 ✅ merged 2026-09-06 (#72), and the deploy ran on 2026-09-07 (D-574). The app is live on `decktome.com`, and Cloud Build deploys every merge (D-584, D-586). The deploy half of its gate stays open on the build walk alone. A reader must upload a collection, then build, revise, and export on the deployed app. PR-23 🔧 built 2026-09-06 on branch `pr-23` (D-553). Each gate holds before the next slice starts. PR-16 ✅ merged 2026-08-29 (#47). PR-16B ✅ merged 2026-08-29 (#48). PR-17 ✅ merged 2026-08-31 (#49). The paid re-baseline of D-302 ran on 2026-08-31.
...
28. **PR-33** one model call that lands in band (F-77, F-78). It runs next. The read of two failed builds and 40 gate decks came first, and `docs/reference/on-band-plan-2026-09-08.md` holds it.
