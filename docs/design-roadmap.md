...
| F-34 | **The bracket cut and the precon share fought, and a failed repair replaced a legal deck.** Deck gate run 14, 2026-09-04: the Goblin Storm upgrade of prompt 17 came back with no cards and a `deck_size` block. The bracket cut of D-468 drops a forbidden shortlist card and keeps only the commanders and the locked cards, so precon cards left the pool. The share rule of D-218 still asked for 67 of 78 names. The repair turn read fewer marked names than the rule demanded, answered no cards, and the builder replaced the legal first deck with the empty one. Run 13b passed the same prompt because its repair did not give up. | 🔧 fixed on branch `deck-gate-fixes`, 2026-09-04 (D-509): the share counts the precon names the pool holds, and a repair with a block or a miss after a clean pass leaves the clean pass in place with the note `repair_kept_earlier`. Two tests pin it, and deck gate run 14b passed the prompt with the fix. |
...
**PR-24: Precon exclusion (D-407 to D-409).** ✅ merged 2026-09-03 (#59, D-496 to D-498, D-500, D-501). The free gate is `docs/reference/pr24-precon-gate-2026-09-03.md`.
