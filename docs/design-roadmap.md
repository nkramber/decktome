...
**PR-32: The commander the reader named, and the pool they chose (F-75, F-76).** 🔧 in progress 2026-09-08. The branch is `pr-32-commander-and-pool`, at commit `a188378`, with no pull request open. **Done:** the runtime read of F-76, and the web half of its fix (D-604). **Left:** F-75 whole, the deck field that carries the ownership of a commander, and the smoke lane that builds from a collection. The first walks of the deployed app found both, and one feedback row names each.

A commander name the index does not know reaches the build and leaves it. The reader wrote "Aragorn as commander", no card carries that name alone, and the build took the delegation path of D-232 and picked its own commander (F-75). The reader delegated nothing. A row asks which card they mean now, as the set row asks (D-376). More than three names match a common word, so the row offers the best three by commander quality at the power the reader picked (owner decision, 2026-09-07). A name that matches nothing says so.
...
The runtime read ran on 2026-09-08, and it settled the cause: not the pool, and not the commander pool. Deck `u8FV7fc98qzNvRfsuJ5q` holds 77 cards, every one owned, and a buy cost of zero. `buyRows` of the web app wrote an ownership that no field holds, and it invents nothing now (D-604). **The deck still carries no ownership for its commander**, so a commander the reader truly lacks reaches no buy list. That half is the work left. The hard filter of OQ-79 needs no code: `CommanderPool` refuses an unowned commander under `POOL_RULE_OWNED_ONLY` already.

The smoke flow of PR-23 covers no build from a collection. It uploads one and then picks "Any card", because its fixture deck fits an any-card shortlist alone (#92). A fixture deck of an owned-first shortlist closes that gap. That lane is the one that meets F-76.
