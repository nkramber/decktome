...
2026-09-04 correction pass 69 (the PR-15 paid gate, D-514, D-515, F-36 to F-39, OQ-62, OQ-63): the three steps of D-513 ran for $4.53. Deck gate run 16 passes 25 of 25, and the compare names no gate flip against run 14 with 14b. Run 16 is the decks baseline with the first complete fingerprint. Tier judge run 4 reads 7 of 25 on the open bar, and the sweep stopped there, so the revise gate waits (OQ-63). Question gate run 34 read 21 misses on the fourth bar. 17 came from the renderer, which read id fields the classifier never fills. 3 came from expectations that named a message that never goes out, and 1 is a wrong slot (F-36). The renderer, the expectations, and the run file of a FAIL are fixed on the branch, and run 35 waits (OQ-62). The plan judge wrote "placeholder" as a reason in 10 of 25 decks (F-38) and graded legality from stale card knowledge (F-39). The Avengers prompt with its precon excluded builds a deck with no superhero identity, and the summary hides it (F-37). The gate document is `docs/reference/pr15-paid-gate-2026-09-04.md`. Changes: D-514, D-515, F-36 to F-39, OQ-62, OQ-63, `cmd/questions-gate/expect.go`, `internal/generate/planjudge.go`, `docs/reference/eval/`.
...
| F-35 | **The whole-precon check counts basic lands, and the export lacks six of them.** The ManaBox deck binder of Avengers Assemble in the export of 2026-08-24 holds 84 of the 90 printings of the product. The six absent cards are basic lands of three printings, Plains 288, Island 290, and Mountain 294 of MSH. Under D-408 that binder owns no precon, so "not from my precons" excludes nothing for it, and a named product gets the partial note of D-497. The exclusion never removes a basic land (D-37), so the six cards change no deck. | 🔧 open, OQ-56 (2026-09-04): the owner decides whether the check ignores basic lands. The deck gate fixture adds the six cards by hand (D-510). |
| F-36 | **The classifier replaces a named theme with a vague later phrase.** Question gate run 34, 2026-09-04: "poison in a tournament" opens with "An infect deck for a Modern tournament", and the second message says "Modern. The best deck under budget." The theme slot ended as `the best deck under budget`, and the word infect was gone. The reader never changed the theme. The deck would be built on the phrase and not on infect. | 🔧 open (2026-09-04): the expectation stays at `infect`, so run 35 names the miss again (OQ-62). The fix is in the classifier: a phrase with no theme word does not replace a named theme. |
...
**PR-15: Eval harness.** 🔧 built 2026-09-04 on branch `pr-15` (D-511, D-512), all six slices. The free gate is `docs/reference/pr15-gate-2026-09-04.md`. The paid gate ran on 2026-09-04 (D-513 to D-515). The deck lane passes, and run 16 is the decks baseline. The question lane and the revise lane wait on two paid runs (OQ-62, OQ-63). `docs/reference/pr15-paid-gate-2026-09-04.md` holds the read with F-36 to F-39. OQ-57 to OQ-63 wait on the owner. The "Tier 1 nightly" line below is a question (OQ-57), and the text stays as written. The sweep runs on the owner's machine under a cap, and CI runs Tier 0 alone.
...
21. PR-15 eval harness. 🔧 built 2026-09-04 on branch `pr-15`, after #64 (D-511), before PR-22 and PR-23. The paid gate ran the same day, and two paid runs complete it (OQ-62, OQ-63). M-5 manual scoring runs on the first UI build (after PR-12).
