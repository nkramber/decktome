...
| OQ-83 | **Does the build read the payoff shape of the commander?** The review of the Gríma deck asks the shortlist to classify a commander first: a trigger that fires once per hit, damage that scales, or a count. Gríma, Saruman's Footman fires once per hit, so a trigger copier or double strike adds more than a pump. The build reads the theme and the top lists today, and not the shape. `docs/reference/deck-review-grima-2026-09-14.md` maps the suggestion. | It changes what the build values for every commander. | Nothing today. PR-53 adds finisher roles first (D-719). |
| OQ-84 | **Does the build discount evasion that the commander already has?** The review asks for a discount on a card whose only job is to grant evasion that the commander holds: can't be blocked, flying, menace, or shadow. The build has no such discount today. | It can drop cards that a reader expects in the deck. | Nothing today. |
| OQ-85 | **Does the build cap each effect class?** The review asks for a curve of diminishing returns per effect class, weighted by what the commander cares about. Its example: seven counterspells are fine, and eight pump equipment on a trigger that fires once are not. The role caps of the shortlist count roles, and no cap counts an effect class. | A cap changes the shape of every deck past the role caps. | Nothing today. |
| OQ-86 | **Does a deck show a power estimate beside its rules bracket?** The review read the deck as bracket 3 by the rules and 2.5 by power, and the build aimed at bracket 5. The profile holds the counts of bracket 4 and 5 floors, and the bracket judge reads a bracket. No reader-facing number shows the power apart from the rules. | It adds a number that every reader sees. | Nothing today. |
