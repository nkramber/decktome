...
|---|---|---|---|
| OQ-79 | An owned-only pool and the commander: must the commander be a card the reader owns? D-63 makes the pool a weak signal for the commander, because the offer goes out before the pool question. A reader who asked for their own cards alone read a deck led by a card to buy (F-76). | It sets what "only cards I own" promises, and D-63 reads the other way today. | Every owned-only build. |
| OQ-80 | Per-deck metrics: which numbers do you want stored? The time of the build, the count of repair turns, the misses and findings of each one, the size of the shortlist, and the model cost are all available and none are stored. The logs hold the repair turns and nothing holds the time. | It sets what the app can learn from, and every number costs a field. | The read of "almost every deck goes to repair", and the read of a slow build. |
