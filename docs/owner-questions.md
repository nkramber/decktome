...
|---|---|---|---|
| OQ-87 | **Which combo rule does the bracket judge follow at bracket 2, and do two precons stay bracket 2 anchors?** The judge text allows no two-card infinite combo at bracket 2. `go/internal/rules/brackets.json` allows one of speed 2 or slower. Precons 5 and 7 of M-15 each hold a speed 5 two-card combo, and the app forbids it at brackets 2 and 3 (F-162). | Each answer moves the target of PR-45 and the anchors of M-15 (D-699). | A later change of the judge text, and the next calibration read. |
