...
3. Continue to chat. Write what you want, for example "a mono-green Commander deck around elves". The agent streams its questions. Click an option, or type an answer in the field under the question. A question stays on the screen until you answer it. The status line says when the build runs, and the spend line shows the session cost (M-1).
