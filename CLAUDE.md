...
Stage (2026-09-14): **the app is live on `decktome.com`**, and pull request #164 is the newest merge. A merge to `main` deploys itself on Cloud Build (D-584, D-586, D-588). Phase 3B, the product UI, is the current phase.
...
**The branch `pr45a-bracket-cut` holds PR-45a** (F-125, D-702). The build cuts one card of each combo its bracket forbids, and each other forbidden card, and a basic land fills each slot. Bracket gate run 3 cut two cards, and 9 of 9 decks at brackets 1 to 3 hold no content violation. Its judge bar still reads FAIL, at 3 of 9.
