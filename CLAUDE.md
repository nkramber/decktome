...
Stage (2026-09-13): **the app is live on `decktome.com`**, and pull request #159 is the newest merge. A merge to `main` deploys itself on Cloud Build (D-584, D-586, D-588). Phase 3B, the product UI, is the current phase.
...
**OQ-82 has an answer** (D-693 to D-698). A bracket promises its power in both directions, and a precon upgrade cuts a kept card that the bracket forbids. M-14 ran bracket gate run 2 on `main` for $1.37, and a commander in the 99 left a block (F-124). PR-46 fixes F-124, and PR-47 gives the judge the Game Changer flags of the card data (F-123). With the flags the judge agrees on 3 of 15 decks, and it still names combos from memory (F-126). M-15 calibrates the judge before PR-45.
