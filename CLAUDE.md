...
Stage (2026-09-14): **the app is live on `decktome.com`**, and pull request #168 is the newest merge. A merge to `main` deploys itself on Cloud Build (D-584, D-586, D-588). Phase 3B, the product UI, is the current phase.
...
**The session built PR-45b, and its pull request waits for the review** (D-709 to D-715). Brackets 4 and 5 take power floors, and the shortlist pins the power cards that the top lists play. A deck that misses a floor names the cards that close it. The fixes of F-131 to F-133 ride along. Bracket gate run 6 reads the judge at 4 of 6, and its second judge lane reads 6 of 6. Before PR-45b the judge read 0 of 6.
