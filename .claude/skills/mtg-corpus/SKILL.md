...
- Sets endpoint `/sets` (verified 2026-08-31, 1,049 rows, one page): `code, name, set_type, released_at, parent_set_code, digital, card_count`. No bulk card file holds `parent_set_code`, so this endpoint is the only source of a set family. `hoc` The Hobbit Eternal carries the parent `hob`, and `ltc` Tales of Middle-earth Commander carries `ltr`. A name prefix can not find the second pair (D-377).
- A card is not in one set. The 2026-08-31 snapshot holds 34,599 playable Oracle cards over 988 paper sets. 16,247 of those cards hold printings in two sets or more. The average is 2.4 sets per card, and the card with the most holds 225.
...
| Format | Always, unless stated. Ask this first. Every other slot depends on it. The app builds three (D-155). The row drops Commander for a reader who asked for a 60-card deck, because Commander is 100 cards. A named card that can lead a deck and plays in neither other format names Commander, and the row does not fire (D-388). | "Which format: Commander, Standard, or Modern?" |
...
| Theme (no card found) | The theme holds words, and no word matches a card of the format and the colors (D-725, D-728). The words "opponent milling cards" matched no card, so the build held staple roles alone (F-142). The row goes out word for word. It asks again only for another theme that matches no card, the D-210 rule. A reply that keeps the theme closes the row, and the build reads the words as they are. | "No card I know matches the theme {theme}. What should the deck do: a creature type, a mechanic, or a play style?" |
...
| Commander (pick) | The row waits for the power, because the offer ranks on the bracket (D-630). It never shares a turn with the theme or the colors question, because the offer ranks on both (D-669). A reader who skips either question still gets the offer on the next turn. The user asked the agent to name a commander (D-71). The row asks again only with names the user has not seen: a refusal retires the old ones (D-73, D-80), and the color check drops the names the colors exclude (D-153). The same three names never go out twice (D-163). | "Which commander do you want: {A}, {B}, or {C}? Say 'none' and I name three more." |
...
| Commander (which card) | The row waits for the power, because the offer ranks on the bracket (D-630). The reader named a commander the card index does not hold, and one or more cards hold that name (F-75, D-606). "Aragorn" is four cards. The row offers the best three by commander quality, at the power the reader picked, and "None of these" (D-607). It asks again only for another name it can not settle, the D-210 rule. | "More than one card holds the name {unknown_commander}. Which one do you want: {commander_options}?" |
| Commander (no such card) | The same as the row above, for a name no card holds at all. The row says so, and it offers no card to pick. | "No card I know holds the name {unknown_commander}. Which commander do you want?" |
...
| Power (60-card) | The user asked for no strong deck. Ask again when the user names a step and also says competitive, strong, best, or serious. Those words conflict with the named step. The row goes out word for word, because the ask role dropped the words that explain the acronym (D-384). | "How strong should this be: casual, FNM level, or tournament-meta?" |
| Colors | The user gave no preference. Never ask when a commander is set. The color identity fills this slot. Never ask while a named card that can lead a deck has no role: that card fixes the identity when it leads (D-388). A delegation such as "surprise me" closes the slot, unless the classifier read it as a commander answer (D-388). "Colorless" is an answer, and it closes the slot (D-165). State no fact about which colors are strongest (D-108). | "Any color preference?" |
| Card pool (precon) | The user asked to upgrade a precon (D-113). This row replaces the row below, and it names the precon. The build keeps 85 percent of the precon's cards, and it reads the product name from the user's own words (D-218, D-247). It never shares a turn with the row "Theme (no card found)". | "Should I build from your {precon} precon first, use only cards from it, or ignore it for a fully optimized deck?" |
| Card pool | The session has a collection (D-37), and the format, the colors, and the theme have values (D-67). It never shares a turn with the row "Theme (no card found)". | "Build from your library first, only your library, or ignore it for a fully optimized deck?" |
| Card pool (thin theme) | `ThinTheme` is set (D-63). This row replaces the row above. It never shares a turn with the row "Theme (no card found)". | "Your library holds {n} {theme} cards. I want 30 or more. Build owned-first with a buy list, or use the whole pool?" |
| Budget | The user mentions cost, the deck needs a buy list, or the pool mode is any-card. A session with no collection always needs a buy list, because the user owns nothing to build from (D-168). Both directions answer the row without a number: "money is no object" and "I can not spend anything" (D-168, D-387). The row asks what limit to use, if any. It must not presume a purchase (D-388). | "What spending limit, if any, should I use for cards you do not own?" |
...
| Set (not resolved) | The reader named a set this app can not settle (D-376). A name it does not hold, or one that names two base sets, such as "Tarkir" or "Strixhaven". The row names the sets it may have meant. It asks again only for another name it can not settle, which is the D-210 rule of the format rows. | "I do not know which set you mean by {bad_set}. Which one: {set_a}, {set_b}, or {set_c}?" |
| Set (mana from outside) | The deck is limited to a set family, and that family holds fewer ramp cards and lands than the role targets want (D-382). Basic lands are never counted: no set limit filters them (D-378). The row goes out word for word, because it names two counts and one permission. | "{sets} hold {n_mana} mana cards, and this deck wants {m_mana}. May I use mana rocks and lands from outside them?" |
| Precon (not resolved) | The reader named a precon the deck must use no card of, and the precon table can not settle the name (D-496). A name the table does not hold, or one that two products with different cards share. The row names the products it may have meant. It asks again only for another name, the D-210 rule. | "I do not know a precon named {bad_precon}. Which one: {precon_options}?" |
