...
| Weak commander pool | An owned mode, and no owned commander fits the theme. The not-owned row is retired: the rules engine reports ownership per card after the build, as a warning in owned-first and a block in owned-only (D-226). | "Your library holds no strong {theme} commander. Buy {C} for {price}, change the theme, or use {A}?" |
