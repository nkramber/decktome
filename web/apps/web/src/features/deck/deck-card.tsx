...
import { deckSize, formatLabel, powerLabel, priceText } from "./deck-stats";
...
export function DeckCard({
  ...
  const size = deckSize(deck.cardCount, deck.commanderOracleIds.length);
  ...
  return (
    <li className="card-hover relative isolate flex flex-col gap-3 overflow-hidden rounded-card border border-border bg-card p-4">
      ...
      {/* The title takes the width, and the two controls sit together at
          the right edge, so the star lands in the same place on every
          tile whatever the title's length (D-455). */}
      <div className="flex items-start gap-2">
        ...
        <Button
          ...
          className="relative z-10 -mt-1 size-7"
        >
      ...
      <div>
        <div className="mb-1 flex justify-between font-mono text-[11px] text-muted-foreground">
          <span>{size > 0 ? `${size} cards` : "No cards"}</span>
        ...
        <div className="h-0.5 rounded-full bg-secondary" aria-hidden="true">
          <span className="block h-full rounded-full bg-primary transition-all" style={{ width: `${Math.min(100, (size / 100) * 100)}%` }} />
        </div>
      </div>
