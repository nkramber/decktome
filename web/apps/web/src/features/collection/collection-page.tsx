...
export function CollectionPage() {
  ...
  // The upload is a dialog, so the page behind it never changes shape
  // while a reader uploads (roadmap PR-18). "Add a collection" sends the
  // reader here to pick a file, and the dialog opens with it (D-342).
  // The effect above clears the mark, and the dialog body mounts after
  // it, so the first value of askForFile is the one that counts.
  const [pickFile] = useState(askForFile);
  const [uploadOpen, setUploadOpen] = useState(pickFile);
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 md:p-6">
      ...
      {/* The dialog holds the report while it stands open. The page keeps
          it after the dialog closes, so the unresolved rows stay to read. */}
      {result && !uploadOpen && <ImportResult result={result} />}
